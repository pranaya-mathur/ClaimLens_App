"""
ClaimLens AI v3.0 - COMPLETE WORKING VERSION
✅ Uses proven, working endpoints from v2.0
✅ All features from original dashboard
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigate",
        ["🎯 AI-Powered Analysis", "📊 Analytics", "🕸️ Fraud Networks"]
    )
...
if "AI-Powered" in page:
    ...
    with doc_col1:
        ...
        if pan_file:
            if pan_file.type.startswith('image'):
    ...
    with doc_col2:
        ...
        if aadhaar_file:
            if aadhaar_file.type.startswith('image'):
...
# Page 2: Analytics
    ...
    try:
        ...
        st.error(f"Error: {str(e)}")
        st.info("Make sure API server is running on http://127.0.0.1:8000")
...
st.markdown(
    ...
    <div style='text-align: center; color: gray;'>
        ClaimLens AI v3.0 | Multi-Modal Fraud Detection | Built with ❤️ | PRODUCTION READY
    </div>
    """,
    unsafe_allow_html=True
)
